...
class GetRestaurantListUseCaseImpl @Inject constructor(
    ...
    override fun invoke(
        query: String,
        sortType: SortType
    ): Flow<DataResource<List<Restaurant>>> = channelFlow {
        send(DataResource.loading()) // 로딩 상태 전송

        try {
            // 1. 레스토랑 리스트 가져오기 (거리순 정렬은 API 내부 처리)
            restaurantRepository.getRestaurantList(query)
                    ...
                    when (restaurantResult) {
                        is DataResource.Success -> {
                            ...
                            val sortedRestaurants = sortRestaurants(
                                restaurantResult.data, sortType
                            )
                            send(DataResource.success(sortedRestaurants))
                        }
