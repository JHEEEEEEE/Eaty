...
class RestaurantInfoViewModel @Inject constructor(
    ...
    private val _getBlogReviewState = MutableStateFlow<UiState<List<BlogReviewModel>>>(UiState.Empty)
    ...
    fun fetchBlogReviews(query: String, loadMore: Boolean = false) {
        // 1. 추가 로딩 조건 확인
        ...
        viewModelScope.launch {
            when (val dataResource = getBlogReviewListUseCase(query, currentPage)) {
