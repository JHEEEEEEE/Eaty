...
1. 어플 실행화면 </br>
2.  Firebase Authentication 연동 → Google 로그인 지원
──────────────────────────────────────────────────────────  

| **3. 홈화면 카테고리별 리스트** | **4. 리스트를 통해 Detail 진입** | **5. 검색창(추천어)으로 음식점 검색** |
|----------------------------------|----------------------------------|--------------------------------|
...
3. 홈 화면에서 카테고리 클릭 시 관련 카테고리의 음식점 리스트가 거리순으로 출력</br>
4. Detail 진입 즉시 현재 사용자의 위치로부터 음식점까지의 최단루트를 제공</br>
5. 자동 완성 검색 지원 → 사용자가 입력한 키워드에 맞춰 추천 음식 제공</br>
예시: '돼' 입력 → '돼지갈비', '돼지국밥' 등 연관 음식 제안

──────────────────────────────────────────────────────────  

| **6. 현재 위치로부터 식당까지 네비 <br> (전화, 카카오 맵 리뷰, 네이버블로그)** | **7. 어플 내 리뷰달기** | **8. 음식점 주변의 환경(날씨, 지하철)**|
|----------------------------------|----------------------------------|---------------------------------------------------------------------|
...
6. Kakao Blog & Local API 연동 → 음식점별 평점, 이용시간, 메뉴, 리뷰 제공
7. 사용자 리뷰 작성 기능 → 방문한 음식점에 솔직한 후기 작성 가능 </br>
 Firestore 실시간 데이터 반영 → 댓글 작성 후 즉시 업데이트
8. 음식점 위치 기반 날씨 및 교통 정보 제공: 식당 방문 계획을 세우고, 주변 교통편을 쉽게 확인 </br>
OpenWeather API & Kakao Local API 연동 → 실시간 날씨 및 교통 정보 제공
...
| **9. 마음에 드는 음식점 찜하기</br>(등록,해제)** |
|----------------------------------|
| ![favorites](https://github.com/user-attachments/assets/bfcf78ed-7d1f-4faa-9e8a-62dfbb141fee) |

9. 자주 가는 음식점 즐겨찾기 등록 및 해제 </br>
 Firebase Firestore 연동 → 사용자별 즐겨찾기 목록 저장 및 관리 </br>
 즐겨찾기 목록에서 음식점 정보 간편 확인
...
| **10. Map 이용 음식점 찾기 <br> (필터, 클러스터링, 마커)** | **11. 마커를 통해 음식점 Detail 진입** |
|----------------------------------|----------------------------------|
...
10. 지도에서 음식점 탐색 → Naver Map API 활용 </br>
 필터 적용 가능 → 카테고리별 음식점 분류</br>
 최대 45개 음식점 마커 표시 → 선택 시 상세 정보 제공</br>
마커 겹치는 문제 해결하기 위해 클러스터링 적용</br>
11. 음식점 마커 선택 시 상세 페이지 이동</br>
...
| **12. FAQ & Notice** | **13. 프로필 수정** | **14. 로그아웃** |
|----------------------------------|----------------------------------|------------------------------------------------|
...
12. FAQ 및 공지사항 확인 가능 </br>
13. 닉네임 및 프로필 사진 변경 지원 </br>
14. Firebase Authentication 연동 → Google 로그아웃 지원
